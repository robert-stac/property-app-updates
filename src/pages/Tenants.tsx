...
const Tenants: React.FC = () => {
  ...
  useEffect(() => {
    const qTenants = query(collection(db, "tenants"), orderBy("name", "asc"));
  ...
  const handleVacateConfirm = async () => {
    ...
    const vacatedRecord = {
      ...
      amountPaid: tenant.lastAmountPaid || 0,
    ...
    try {
      await addDoc(collection(db, "vacatedTenants"), vacatedRecord);
  ...
  const deleteTenant = (tenant: any) => {
    setVacateModal({ show: true, tenant, reason: "" });
  };
      ...
      <div className="flex flex-col md:flex-row justify-between items-end md:items-center gap-4 bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-1 font-medium">
            1 USD = {(exchangeRate || 0).toLocaleString()} UGX
          </p>
        </div>
          ...
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="divide-y divide-gray-100">
              {filteredTenants.map((tenant) => {
                ...
                // NEW: Dynamic Arrears Calculation for Display
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4 z-[70]">
          <div className="bg-white rounded-2xl w-full max-w-sm shadow-2xl overflow-hidden border border-gray-100">
            <div className="p-6 text-center">
              ...
              <p className="text-sm text-gray-500 mt-2">This will move <b>{vacateModal.tenant?.name}</b> to the archive. They will no longer appear in active units.</p>
            </div>
            <div className="px-6 pb-6 space-y-4">
              <div>
                <label className="text-[10px] font-bold text-gray-400 uppercase ml-1">Reason for Vacating</label>
                <select 
                  className="w-full p-3 bg-gray-50 border border-gray-200 rounded-xl outline-none text-sm font-medium"
                  value={vacateModal.reason}
                  onChange={(e) => setVacateModal({...vacateModal, reason: e.target.value})}
                >
              ...
              <div className="flex gap-3">
                <button onClick={() => setVacateModal({show: false, tenant: null, reason: ""})} className="flex-1 py-3 bg-gray-100 text-gray-600 rounded-xl font-bold hover:bg-gray-200 transition-all">Cancel</button>
                <button onClick={handleVacateConfirm} className="flex-1 py-3 bg-red-600 text-white rounded-xl font-bold hover:bg-red-700 shadow-lg active:scale-95 transition-all">Confirm</button>
              </div>
            </div>
          </div>
        </div>
            ...
            <div className="p-6 space-y-4">
              ...
              <div className="space-y-3">
                <div className="relative">
                  <span className="absolute left-3 top-3 text-gray-400 text-xs font-bold uppercase">USD</span>
                  <input 
                    type="number" 
                    className="w-full pl-12 p-3 bg-gray-50 border border-gray-200 rounded-xl outline-none font-bold"
                    placeholder="0.00"
                    value={(paymentModal.amount / (exchangeRate || 3800)).toFixed(2)}
                    onChange={(e) => setPaymentModal({...paymentModal, amount: Number(e.target.value) * (exchangeRate || 3800)})}
                  />
                </div>
                <div className="relative">
              ...
              <div className="flex gap-3 pt-2">
                <button onClick={() => setPaymentModal({show: false, tenant: null, amount: 0})} className="flex-1 py-3 bg-gray-100 text-gray-600 rounded-xl font-bold hover:bg-gray-200 transition-all">Cancel</button>
                <button onClick={handleProcessPayment} className="flex-1 py-3 bg-emerald-600 text-white rounded-xl font-bold hover:bg-emerald-700 shadow-lg active:scale-95 transition-all">Confirm</button>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-2xl w-full max-w-md shadow-2xl overflow-hidden border border-gray-100">
            <div className="bg-gray-50 px-6 py-4 border-b border-gray-100 flex justify-between items-center">
              ...
              <button onClick={() => setShowModal(false)} className="text-gray-400 hover:text-gray-600 w-8 h-8 flex items-center justify-center rounded-full hover:bg-gray-200">✕</button>
            </div>
            
            <div className="p-6 space-y-4">
              <div className="space-y-1">
                <label className="text-xs font-bold text-gray-500 uppercase">Tenant Name</label>
                <input type="text" className="w-full p-3 bg-gray-50 border border-gray-200 rounded-xl outline-none" value={newTenant.name} onChange={(e) => setNewTenant({ ...newTenant, name: e.target.value })} />
              </div>
              
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <input type="text" placeholder="Contact" className="w-full p-3 bg-gray-50 border border-gray-200 rounded-xl outline-none" value={newTenant.contact} onChange={(e) => setNewTenant({ ...newTenant, contact: e.target.value })} />
                ...
                <select 
                  className="w-full p-3 bg-blue-50 border border-blue-100 text-blue-900 rounded-xl outline-none font-bold text-sm" 
                  value={newTenant.rentFrequency} 
                  onChange={(e) => setNewTenant({ ...newTenant, rentFrequency: e.target.value })}
                >
                  <option value="Monthly">Monthly</option>
                  <option value="3 Months">Quarterly (3m)</option>
                  <option value="6 Months">Bi-Annual (6m)</option>
                  <option value="Yearly">Yearly</option>
                </select>
              </div>

              <div className="space-y-2">
                <label className="text-xs font-bold text-gray-500 uppercase ml-1">Total Rent Amount</label>
                <div className="grid grid-cols-2 gap-4">
                  <div className="relative">
                    <span className="absolute left-3 top-3.5 text-gray-400 text-xs font-bold">USD</span>
                    <input 
                      type="number" 
                      className="w-full pl-12 p-3 bg-blue-50 border border-blue-100 rounded-xl outline-none font-bold text-blue-900" 
                      value={(newTenant.rentAmount / (exchangeRate || 3800)).toFixed(2)} 
                      onChange={(e) => handleAmountChange(e.target.value, "USD", "rentAmount")} 
                    />
                  </div>
                  <div className="relative">
                    ...
                    <input 
                      ...
                      onChange={(e) => handleAmountChange(e.target.value, "UGX", "rentAmount")} 
                    />
                  </div>
                </div>
              </div>
              ...
              <div className="space-y-2">
                <label className="text-xs font-bold text-gray-500 uppercase ml-1">Paid So Far</label>
                <div className="grid grid-cols-2 gap-4">
                  <div className="relative">
                    <span className="absolute left-3 top-3.5 text-emerald-400 text-xs font-bold">USD</span>
                    <input 
                      type="number" 
                      className="w-full pl-12 p-3 bg-emerald-50 border border-emerald-100 rounded-xl outline-none font-bold text-emerald-800" 
                      value={(newTenant.lastAmountPaid / (exchangeRate || 3800)).toFixed(2)} 
                      onChange={(e) => handleAmountChange(e.target.value, "USD", "lastAmountPaid")} 
                    />
                  </div>
                  <div className="relative">
                    <span className="absolute left-3 top-3.5 text-emerald-400 text-xs font-bold">UGX</span>
                    <input 
                      type="number" 
                      className="w-full pl-12 p-3 bg-emerald-50 border border-emerald-200 rounded-xl outline-none font-bold text-emerald-800" 
                      value={newTenant.lastAmountPaid} 
                      onChange={(e) => handleAmountChange(e.target.value, "UGX", "lastAmountPaid")} 
                    />
                  </div>
                </div>
              </div>
              ...
              <div>
                   ...
                   <input type="date" className="w-full p-3 bg-gray-50 border border-gray-200 rounded-xl outline-none" value={newTenant.paidUntil} onChange={(e) => setNewTenant({ ...newTenant, paidUntil: e.target.value })} />
              </div>

              <button onClick={saveTenant} className="w-full py-3 bg-blue-900 text-white rounded-xl font-bold hover:bg-blue-800 transition-all shadow-md active:scale-95">Save Tenant Record</button>
            </div>
          </div>
        </div>
      )}
